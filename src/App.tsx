...
export default function App() {
  ...
  // Initialize/Load Notifications — no mock injection; clean start shows 0
  useEffect(() => {
    try {
  ...
  // Reports come from PostgreSQL only. The old localStorage cache had no
  // writer left, so reading it could only ever surface stale rows.
  useEffect(() => {
    // Load trends
    try {
      const storedTrends = localStorage.getItem(TREND_STORAGE_KEY);
      setTrendData(storedTrends ? JSON.parse(storedTrends) : []);
    } catch (e) {
  ...
  // We no longer inject mock reports/trends into localStorage.
  // Reports and trends are persisted to PostgreSQL via /api/save-analysis-result
  // and fetched via /api/diagnosis-logs and /api/trends endpoints.
...
const updated = [newReport, ...reports];
    ...
    // Diagnostic report saved to state; persistence is now via /api/save-analysis-result
    ...
    // Attempt to extract vibration, temperature from symptoms text or preset them realistically
    let vibe = 1.5;
    let temp = 55.0;
        ...
        >
          ...
          {selectedReport ? (
            ...
            <AnalysisReport
              ...
              onNavigateToRca={() => navigateToTab("rca")}
            />
          ) : activeTab === "rca" ? (
