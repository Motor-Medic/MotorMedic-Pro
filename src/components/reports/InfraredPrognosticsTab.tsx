...
export default function InfraredPrognosticsTab({ isActive, selectedAnalysis, loadedAnalyses }: Props) {
  ...
  const exclusion = useMemo(
    () => componentExclusionVerdict(assetRuns, component),
    [assetRuns, component],
  );
      ...
      <div className="bg-slate-800/50 border border-slate-700/60 rounded-lg p-3 space-y-2">
        ...
        {exclusion.count > 0 && (
          <p className="text-xs text-amber-400">
            {exclusion.count} stored run{exclusion.count === 1 ? "" : "s"} excluded - mismatched sub-component ({exclusion.components.join(", ")}); series scoped to tracked component "{component}"
          </p>
        )}
